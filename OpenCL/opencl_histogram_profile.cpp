...
void serial_histogram(Image *img,uint32_t R[256],uint32_t G[256],uint32_t B[256]){
    std::fill(R, R+256, 0);
    std::fill(G, G+256, 0);
    std::fill(B, B+256, 0);
...
void printBin(unsigned int *ptr) {
    unsigned int min=ptr[0], max=ptr[0], sum=0;
    int cnt = 0;
    ...
    std::cout << min << "," << sum/256 << "," << max << std::endl;
}
...
void evt_end(const char* evt_name) {
    end = std::chrono::steady_clock::now();
    std::cout << std::fixed << (double)std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count()/(float)1e+9 << " [sec] " << evt_name << std::endl;
}
...
int main(int argc, char *argv[])
{
    ...
    cl_command_queue myque = clCreateCommandQueueWithProperties(myctx, device, props, &clError);
    if(clError != CL_SUCCESS) {
        printf("clCreateCommandQueueWithProperties fails (%d)\n", clError);
        return -1;
    }
    evt_end("prepare environemnt");
    ...
    if (argc >= 2)
    {
        ...
        for (int i = 0; i < many_img; i++)
        {
            ...
            clWaitForEvents(1, &hist_evt);
            std::cout  << "\t" << std::fixed << executionTime(hist_evt) << " [sec]"<< " (enqueue_hist)"<< std::endl;
            clWaitForEvents(1, &img_evt);
            std::cout  << "\t" << std::fixed << executionTime(img_evt)  << " [sec]" << " (enqueue_img)" << std::endl;
            ...
            // Create Kernel (cal_partial_histogram)
            ...
            // Execute Kernel (kernel_cal_partial)
            ...
            clWaitForEvents(1, &cal_evt);
            std::cout << "\t" << std::fixed << executionTime(cal_evt) << " [sec]" << " (kernel cal_partial)"<< std::endl;
            ...
            clWaitForEvents(1, &comb_evt);
            std::cout  << "\t" << std::fixed << executionTime(comb_evt) << " [sec]" << " (kernel comb_partial)"<< std::endl;
            ...
            clWaitForEvents(1, &read_hist_evt);
            std::cout << "\t" << std::fixed << executionTime(read_hist_evt) << " [sec]"<< " (read_hist_evt)"<< std::endl;
            
            // OpenCL Program Result
            for(int i=0; i<256; i++){
            ...
            std::cout << "\nImage Shape:"<< img->weight << "x" << img->height << "\n";
            std::cout << "global_work_size : " <<  global_work_size << "\n";
            std::cout << "local_work_size : " <<  local_work_size << "\n";
            std::cout << "num_group : " <<  num_group << "\n";
        }
    }else{
